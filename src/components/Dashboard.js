...
const Dashboard = ({ country = 'us', category = 'general', pageSize = 8 }) => {
  ...
  useEffect(() => {
    ...
    setArticles([]); // Clear existing articles when country/category changes
    fetchNews(page); // Fetch news on initial load or when dependencies change

    // The effect will re-run when country, category, pageSize, or page changes
